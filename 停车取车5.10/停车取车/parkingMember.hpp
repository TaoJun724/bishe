...
public:
	...
	void change(int x, int y, int length, int wide)//对占用的便道/停靠点/充电点进行改值
	{
	...
	void restore(int x, int y, int length, int wide)//恢复便道值
	{
		for (int i = x - wide + 1; i <= x; i++)//进行局部的赋值
		{
			for (int j = y; j <= y + length - 1; j++)
	...
	void restore1(int x, int y, int length, int wide)//表示正在离开停靠点
	{
		for (int i = x - wide + 1; i <= x; i++)//进行局部的赋值
		{
			for (int j = y; j <= y + length - 1; j++)
	...
	void restore2(int x, int y, int length, int wide)//恢复停靠值
	{
		for (int i = x - wide + 1; i <= x; i++)//进行局部的赋值
		{
			for (int j = y; j <= y + length - 1; j++)
...
public:
   ...
   const char* position;
	bool status;
	string license;
	stack<char*> s1;//存取停车时出口到车位的路径
	stack<char*> s2;//存取停车时停靠点到出口的路径
	stack<string> s3;//存取停靠时的路径
	stack<string> position0;//存取停靠位置
};
